    ...
    <Menu.Root open={open} onOpenChange={setOpen}>
      ...
      >
        ...
        <div className="hidden text-left sm:block">
          <p className="text-foreground max-w-30 truncate text-xs font-medium leading-tight">
        ...
        <span className={`hidden rounded px-1.5 py-0.5 text-[10px] font-semibold sm:inline-block ${roleBadgeClass}`}>
          {tRole(user.role)}
        </span>
        <ChevronDown
        ...
        <Menu.Positioner side="bottom" align="end" sideOffset={6} className="z-200">
          <Menu.Popup className="bg-popover border-border text-popover-foreground w-48 rounded-xl border p-1.5 shadow-md outline-none">
            ...
            <div className="px-2.5 py-2">
              ...
              <span className={`mt-1 inline-block rounded px-1.5 py-0.5 text-[10px] font-semibold ${roleBadgeClass}`}>
                {tRole(user.role)}
              </span>
            </div>
